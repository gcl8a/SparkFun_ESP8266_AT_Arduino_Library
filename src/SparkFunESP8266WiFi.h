...
public:
    ...
    int16_t tcpSendPacket(const char *buf);
    int16_t tcpSendPacket(const String&);
...
private:
	...
	int16_t readForResponse(const char * rsp, unsigned long timeout);
	int16_t readForResponses(const char * pass, const char * fail, unsigned long timeout);
